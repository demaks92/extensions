...
local function theme_switcher(opts)
   ...
   if next(themes) ~= nil then
      ...
      local current_theme = vim.g.ui_theme
      local new_theme = ""
      local change = false
      ...
      picker.close_windows = function(status)
         ...
         vim.g.ui_theme = final_theme
         close_windows(status)
      end
      -- launch the telescope picker
      picker:find()
   else
      fn.error("No themes found in " .. vim.fn.stdpath "config" .. "/lua/" .. vim.fn.fnamemodify(vim.g.ui_themes_path, ":r") .. "/themes")
   end
end
...
if present then
   ...
   fn.error("Cannot find telescope!")
end
